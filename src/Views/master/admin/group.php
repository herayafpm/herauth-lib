...
<script>
    ...
    methodsVue = {
        ...
        async getGroups() {
            ...
            await axios.post("<?= $url_groups ?>", formData, {
                validateStatus: () => true
            }).then((res) => {
                if (res.status !== 200) {
                    this.alertType = 'danger'
                    ...
                    if (res.data.status) {
                        ...
                        var data = res.data.data.map((el) => {
                            el.checked = false
                            if (this.admin_groups.filter((elag) => elag.group_id === el.id).length > 0) {
                                el.checked = true
                            }
                            return el
                        })
        ...
        async getAdminGroups() {
            ...
            await axios.post("<?= $url_admin_groups ?>", formData, {
                validateStatus: () => true
            }).then((res) => {
                if (res.status !== 200) {
                    this.alertType = 'danger'
                    ...
                    if (res.data.status) {
                        this.alertType = 'success'
                        this.messageApi = res.data.message
                        this.admin_groups = res.data.data
                    }
                }
                window.scrollTo(0, 0)
            })
