...
<script>
    ...
    methodsVue = {
        ...
        async prosesDeleteUserGroup(username) {
            this.loadingApi = true
            this.cleanForm()

            await axios.post("<?= $url_delete_user_group ?>", {
                username
            }, {
                validateStatus: () => true
            }).then((res) => {
        ...
        async deleteUserGroup(username) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    this.prosesDeleteUserGroup(username)
                }
            })
        },
        async proses() {
            ...
            await axios.post("<?= $url_add_user_group ?>", formData, {
                validateStatus: () => true
            }).then((res) => {
                if (res.status !== 200) {
                    this.alertType = 'danger'
        ...
        async getUserGroups() {
            ...
            await axios.post("<?= $url_user_groups ?>", formData, {
                validateStatus: () => true
            }).then((res) => {
                if (res.status !== 200) {
                    this.alertType = 'danger'
                    ...
                    if (res.data.status) {
                        ...
                        tableUserGroup.on('order.dt page.dt', function() {
                            tableUserGroup.column(0, {
                                order: 'applied',
                                page: 'applied',
                        ...
                        $("#tableUserGroup").on('click', '.hapusData', function() {
